...
*/
function calc(formula,molarity,solution) {
 let mass = 0;
...
function getNumberStartingAt(i, formula) {
   ...
   function displayResult(atoms, mass) {
    ...
    let atomsstr = `<br>Molar Mass of the ${atomsstri} is: ${total}`;

     resultContainer.innerHTML = `Take ${mass} grams of ${atomsstri}`;
    // atoms.forEach(atom => {
    // atomsstr += ' ' + atom;
    // });
